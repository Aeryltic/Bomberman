...
void GoapPlanner::addToOpenList(ANode *node)
{
...
bool GoapPlanner::memberOfClosed(const WorldState& ws)
{
...
std::list<ANode*>::iterator GoapPlanner::memberOfOpen(const WorldState& ws)
{
...
//    open.insert(nodes.front());
    addToOpenList(&nodes.back());

    while(!open.empty())
    {
        ANode* current = popAndClose();
        ...
        for(auto& p : agent->get_actions())
        {
            ...
            if(potential_action.is_doable(current->ws))
            {
                ...
                {
                    //printf("-adding node\n");
                    nodes.push_back(ANode(outcome, current->g + potential_action.getCost(), outcome.distanceTo(goal), current, &potential_action));
                ...
                else
                {
                    ...
                    if(current->g + potential_action.getCost() < (*p_outcome_node)->g)
                    {
                        ...
                        (*p_outcome_node)->g = current->g + potential_action.getCost();
